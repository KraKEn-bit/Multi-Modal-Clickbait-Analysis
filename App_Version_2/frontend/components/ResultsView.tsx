...
export function ResultsView({ result, researchNote }: Props) {
  ...
  const strokeColor = isClickbait ? "#ff5c47" : "#00ffa3";
  ...
  return (
    ...
    >
      <motion.div
        ...
        className={`transform-gpu rounded-2xl border p-8 ${verdictBg}`}
      >
        <p className="text-xs font-semibold uppercase tracking-[0.2em] text-muted">
          Full VTCF verdict
        </p>
        <motion.p
          className={`mt-2 text-5xl font-bold tracking-tight ${verdictColor}`}
      ...
      {result.category === "hard" && result.text_only && (
        <motion.section
          ...
          className="transform-gpu rounded-2xl border-2 border-accent/40 bg-accent/5 p-6"
        >
      ...
      <motion.section
        ...
        className="transform-gpu rounded-xl border border-border bg-surface p-6"
      >
      ...
      <motion.section
        ...
        className="transform-gpu rounded-xl border border-border bg-surface-elevated p-6"
      >
        <h3 className="text-sm font-semibold uppercase tracking-wider text-muted">
          Model explanation
        </h3>
