"""
Final held-out confirm: SECE + ERA5 Phase 3 with environ subset (locked architecture).

Uses HELDOUT_SEEDS only — never DEV_SEEDS. Writes to results/sece_era5_environ_heldout/.
"""
...
OUT_DIR = ROOT / "results" / "sece_era5_environ_heldout"
...
def _paths() -> dict[str, Path]:
    return {
        "raw": OUT_DIR / "heldout_multiseed_phase3_raw.csv",
        "stability": OUT_DIR / "heldout_multiseed_phase3_stability.csv",
        "progress": OUT_DIR / "heldout_multiseed_phase3_progress.json",
        "complete": OUT_DIR / "heldout_multiseed_phase3_complete.flag",
    }
...
def _validate_seeds(seeds: list[int]) -> None:
    for s in seeds:
        if s in DEV_SEEDS:
            raise ValueError(f"Seed {s} is DEV-only; held-out eval must use HELDOUT_SEEDS")
        if s not in HELDOUT_SEEDS:
            raise ValueError(f"Seed {s} is not in HELDOUT_SEEDS {HELDOUT_SEEDS}")


def run_heldout_eval(seeds: list[int] | None = None, resume: bool = False) -> pd.DataFrame:
    OUT_DIR.mkdir(parents=True, exist_ok=True)
    seeds = list(seeds or HELDOUT_SEEDS)
    ...
    print("HELD-OUT confirm: SECE+ERA5 Phase 3 environ subset (locked architecture)", flush=True)
    ...
    meta = {
        "eval_type": "heldout_final_confirm",
        ...
        "dev_reference": "results/sece_era5_environ/dev_multiseed_phase3_raw.csv",
    }
    write_progress(
        paths["progress"],
        {
    ...
    for i, seed in enumerate(pending, start=1):
        ...
        try:
            rank_df = sece_v2_train.run_sece_v2(phase=3, quick=False, seed=seed, quiet=True)
            rows = [
                {
                    "seed": seed,
                    "horizon": row["horizon"],
    ...
    paths["complete"].write_text(f"complete {_utc_now()}\n", encoding="utf-8")
...
if __name__ == "__main__":
    ...
    try:
        run_heldout_eval(seeds=args.seeds, resume=args.resume)
    except Exception:
        sys.exit(2)
