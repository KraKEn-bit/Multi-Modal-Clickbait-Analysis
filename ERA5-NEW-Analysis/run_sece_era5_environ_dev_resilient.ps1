# Resilient SECE+ERA5 Phase 3 WITH environ subset dev eval.
$ErrorActionPreference = "Continue"
$Here = Split-Path -Parent $MyInvocation.MyCommand.Path
$LogDir = Join-Path $Here "results\sece_era5_environ\logs"
New-Item -ItemType Directory -Force -Path $LogDir | Out-Null

$LogFile = Join-Path $LogDir ("dev_eval_resilient_{0:yyyyMMdd_HHmmss}.log" -f (Get-Date))
$PyScript = Join-Path $Here "scripts\sece_era5_environ_dev_eval.py"
$ProgressJson = Join-Path $Here "results\sece_era5_environ\dev_multiseed_phase3_progress.json"
$DoneFlag = Join-Path $Here "results\sece_era5_environ\dev_multiseed_phase3_complete.flag"
$ResumeCmd = Join-Path $Here "resume_sece_era5_environ_dev.cmd"
$TaskName = "Research1_NEWWAY_SECE_ERA5_ENVIRON"
...
if (Test-Complete) {
    Write-Log "Environ-subset dev eval already complete."
    exit 0
}
...
try {
    ...
    Write-Log "Use resume_sece_era5_environ_dev.cmd after power cut"
}

Write-Log "Environ-subset dev eval started. Log: $LogFile"

while ($true) {
    if (Test-Complete) { break }
    Write-Log "Running sece_era5_environ_dev_eval.py --resume"
